...
async function getThumbhashDataURL(imageUrl) {
    try {
        ...
        // 验证响应内容
        if (!thumbhashString || thumbhashString.trim().length === 0) {
            throw new Error('Empty thumbhash response');
        }
...
const main = async (html_content, cfg = {}) => {
    ...
    // 统计支持的图片数量
    const supportedImages = matches.filter(([, img_src]) =>
        isImageSupported(img_src, config.supported_domains, config.exclude_formats)
    );

    console.log("[bitiful_toolkit] Estimated Thumbhash Requests Count:", supportedImages.length);

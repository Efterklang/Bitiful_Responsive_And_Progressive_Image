...
async function getDataURL(imageUrl, cache = null) {
    try {
        ...
        if (cache) {
            ...
            if (cachedDataURL) {
                console.log(`[bitiful_toolkit] ${imageUrl} 缓存命中`);
                return cachedDataURL;
            }
        }

        console.log(`[bitiful_toolkit] ${imageUrl} 缓存未命中，正在获取...`);
        ...
        // 存入缓存
        if (cache && dataURL) {
            console.log(`[bitiful_toolkit] 将${imageUrl}及其对应的dataUrl写入缓存`);
...
const main = async (html_content, cfg = {}) => {
    ...
    console.log("[bitiful_toolkit] Estimated Thumbhash Request Count for this post:", supportedImages.length);

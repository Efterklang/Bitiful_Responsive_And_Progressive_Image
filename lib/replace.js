...
function buildImageHTML(
	...
	if (dimensions?.width) {
		picStyle.push(`max-width: ${dimensions.width}px`);
		picStyle.push(`aspect-ratio: ${dimensions.width} / ${dimensions.height}`);
	}

	const imgTag = `<img src="${img_src}"
        srcset="${srcset}"
        ${shouldLazyLoad ? 'loading="lazy" decoding="async"' : ""}
        style="width:100%; height:100%; object-fit:cover; transition: opacity 0.3s; ${dataURL ? "opacity:0;" : ""}"
